...
```php
listCarriers($org_id, $page, $limit, $brand_id, $brand_scope): \Zippendo\Sdk\Model\ListCarriers200Response
```
...
try {
    $result = $apiInstance->listCarriers($org_id, $page, $limit, $brand_id, $brand_scope);
    print_r($result);
} catch (Exception $e) {
