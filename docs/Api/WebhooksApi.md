...
```php
listOrgWebhooks($org_id, $page, $limit, $brand_id, $brand_scope, $is_active, $search): \Zippendo\Sdk\Model\ListOrgWebhooks200Response
```
...
try {
    $result = $apiInstance->listOrgWebhooks($org_id, $page, $limit, $brand_id, $brand_scope, $is_active, $search);
    print_r($result);
} catch (Exception $e) {
